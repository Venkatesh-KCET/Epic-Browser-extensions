...
// #agad - receive message from Umbrella Popup
chrome.extension.onMessageExternal.addListener(function(request, sender, sendResponse) 
{
  const host = util.getNormalisedHostname(request.url);
  if (request.greeting == "isWhitelistedSite"){
    sendResponse(disabledList.has(host))
    return true;
  }
  // // always validate hostname before adding it to the disabled list
  // if (util.isValidHostname(host)) {
  ...
  if (request.value == true){
    ...
    disabledList.delete(host);
  }
  return sendResponse({success:false});
  // chrome.tabs.query({active:true}, function(tab){
  //   var message={};
  //   message.value=request.value;                                                                            
  //   // localStorage.setItem(GetDomain(tab.url),JSON.stringify(message)); 
  // });
});
