    ...
    return async function(tabId, details) {
        ...
        if ( browserAction.setIcon !== undefined ) {
            ...
            if ( (parts & 0b0010) !== 0 ) {
                ...
                sendToUmbrellaBadgeTextThrottled(tab.id, (parts & 0b1000) === 0 ? badge : '')
            }
            if ( (parts & 0b0100) !== 0 ) {
...
vAPI.messaging = {
    ...
    onPortConnect: function(port, isExternal=false) {
        ...
        // #agad - port privilege is set to true in cross-extension communication
        portDetails.privileged = isExternal||url.startsWith(this.PRIVILEGED_URL);
        if ( tab ) {
    ...
    setup: function(defaultHandler) {
        ...
        // #agad - allow cross-extension communication
        browser.runtime.onConnectExternal.addListener(
            port => this.onPortConnect(port,true)
        );
...
let umbrellaExtId = null;
async function sendToUmbrellaBadgeText(tabId, text){
    if (umbrellaExtId == null){
        umbrellaExtId = await getExtensionIdByName("03_Umbrella Button")
    }
    if (!umbrellaExtId) return;
    await promisify(r => chrome.runtime.sendMessage(umbrellaExtId, {ext:"setBadgeText", tabId, text}, r));
}

const sendToUmbrellaBadgeTextThrottled = throttle(sendToUmbrellaBadgeText,50);
