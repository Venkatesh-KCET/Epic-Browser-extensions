...
/******************************************************************************/
let hiddenWhitelist = []
function removedHiddenWhitelistedSites(whitelist){
    let hiddenSites = whitelist.filter(e => e.startsWith("hidden")).map(e => e.replace(/^hidden\./, ""));
    let hiddenSitesMap = {}
    hiddenSites.forEach(e => {
        hiddenSitesMap[e]=true
        hiddenSitesMap[`hidden.${e}`]=true
    });
    hiddenWhitelist = Object.keys(hiddenSitesMap)
    return whitelist.filter(site => !(site in hiddenSitesMap));
}
const renderWhitelist = async function() {
    let details = await messaging.send('dashboard', {
        what: 'getWhitelist',
    });
    // #agad - debug
    // console.log("details.whitelist", details.whitelist);
    details.whitelist = removedHiddenWhitelistedSites(details.whitelist)
