...
chrome.extension.sendRequest({reqtype: "get-settings", iframe: iframe}, function(response) {
	...
	if (typeof response === 'object' && response.status == 'true') {
		...
		console.log("before SETTINGS['AUDIOBLOCK'] ="+SETTINGS['AUDIOBLOCK']);
		SETTINGS['AUDIOBLOCK'] = response.audioblock;
		console.log("after SETTINGS['AUDIOBLOCK'] ="+SETTINGS['AUDIOBLOCK']);
		...
		if (SETTINGS['CANVAS'] != 'false' || SETTINGS['CANVASFONT'] == 'true' || SETTINGS['CLIENTRECTS'] == 'true' || SETTINGS['AUDIOBLOCK'] == 'true' || SETTINGS['BATTERY'] == 'true' || SETTINGS['WEBGL'] == 'true' || SETTINGS['WEBRTCDEVICE'] == 'true' || SETTINGS['GAMEPAD'] == 'true' || SETTINGS['WEBVR'] == 'true' || SETTINGS['TIMEZONE'] != 'false' || SETTINGS['CLIPBOARD'] == 'true') {
            console.log("calling palak fingerprintProtection");
			fingerprintProtection();
		}
...
function loaded() {
	BrowseSafe();
	new MutationObserver(BrowseSafe).observe(document.querySelector("body"), { childList: true, subtree : true, attributes: false, characterData : false });
}
function BrowseSafe() {
	if (SETTINGS['LINKTARGET'] != 'off' || SETTINGS['DATAURL'] == 'true' || SETTINGS['REFERRER'] == 'alldomains' || (SETTINGS['REFERRER'] == 'true' && (SETTINGS['DOMAINSTATUS'] != '0' || SETTINGS['REFERRERSPOOFDENYWHITELISTED'] == 'true'))) {
		$("a[data-ss"+timestamp+"!='1']").each(function() {
			...
			if (SETTINGS['DATAURL'] == 'true' && elSrc.match(/^\s*data:text\//i)) {
				...
				attr['target'] = '';
				attr['href'] = 'data:text/html,<h1>This data:text/html link has been sanitized by BrowseSafe.</h1><p>Original link:<br><strong>'+$(this).attr('href').replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/^\s*data:text/i, "data-BROWSESAFE:text")+'</strong></p><p>If you would like to still load it (not recommended), copy and paste the above string into your address bar and remove "-BROWSESAFE" which is inserted as a safeguard.</p><p><a href="javascript:history.go(-1);">Go Back</a></p>';
			}
	...
	/*if (SETTINGS['CANVAS'] != 'false') {
		$("canvas.browsesafe_heedlljjfegnjeijpnkbhpofeejflkea_canvas").each(function() { chrome.extension.sendRequest({reqtype: "update-blocked", src: window.location.href+" ("+$(this).attr('title')+"())", node: 'Canvas Fingerprint'}); $(this).remove(); });
	}
	if (SETTINGS['CLIPBOARD'] == 'true') {
		$("div.browsesafe_heedlljjfegnjeijpnkbhpofeejflkea_clipboard").each(function() { chrome.extension.sendRequest({reqtype: "update-blocked", src: window.location.href+" ("+$(this).attr('title')+"())", node: 'Clipboard Interference'}); $(this).remove(); });
	}
	if (SETTINGS['CANVASFONT'] == 'true') {
		$("div.browsesafe_heedlljjfegnjeijpnkbhpofeejflkea_canvasfont").each(function() { chrome.extension.sendRequest({reqtype: "update-blocked", src: window.location.href+" ("+$(this).attr('title')+"())", node: 'Canvas Font Access'}); $(this).remove(); });
	}
	if (SETTINGS['AUDIOBLOCK'] == 'true') {
		$("div.browsesafe_heedlljjfegnjeijpnkbhpofeejflkea_audio").each(function() { chrome.extension.sendRequest({reqtype: "update-blocked", src: window.location.href+" ("+$(this).attr('title')+"())", node: 'Audio Fingerprint'}); $(this).remove(); });
	}
	if (SETTINGS['WEBGL'] == 'true') {
		$("div.browsesafe_heedlljjfegnjeijpnkbhpofeejflkea_webgl").each(function() { chrome.extension.sendRequest({reqtype: "update-blocked", src: window.location.href+" ("+$(this).attr('title')+"())", node: 'WebGL Fingerprint'}); $(this).remove(); });
	}
	if (SETTINGS['BATTERY'] == 'true') {
		$("div.browsesafe_heedlljjfegnjeijpnkbhpofeejflkea_battery").each(function() { chrome.extension.sendRequest({reqtype: "update-blocked", src: window.location.href+" ("+$(this).attr('title')+"())", node: 'Battery Fingerprint'}); $(this).remove(); });
	}
	if (SETTINGS['WEBRTCDEVICE'] == 'true') {
		$("div.browsesafe_heedlljjfegnjeijpnkbhpofeejflkea_webrtc").each(function() { chrome.extension.sendRequest({reqtype: "update-blocked", src: window.location.href+" ("+$(this).attr('title')+"())", node: 'Device Enumeration'}); $(this).remove(); });
	}
	if (SETTINGS['GAMEPAD'] == 'true') {
		$("div.browsesafe_heedlljjfegnjeijpnkbhpofeejflkea_gamepad").each(function() { chrome.extension.sendRequest({reqtype: "update-blocked", src: window.location.href+" ("+$(this).attr('title')+"())", node: 'Gamepad Enumeration'}); $(this).remove(); });
	}
	if (SETTINGS['WEBVR'] == 'true') {
		$("div.browsesafe_heedlljjfegnjeijpnkbhpofeejflkea_webvr").each(function() { chrome.extension.sendRequest({reqtype: "update-blocked", src: window.location.href+" ("+$(this).attr('title')+"())", node: 'WebVR Enumeration'}); $(this).remove(); });
	}
	if (SETTINGS['CLIENTRECTS'] == 'true') {
		$("div.browsesafe_heedlljjfegnjeijpnkbhpofeejflkea_clientrects").each(function() { chrome.extension.sendRequest({reqtype: "update-blocked", src: window.location.href+" ("+$(this).attr('title')+"())", node: 'Client Rectangles'}); $(this).remove(); });
	}
	if (SETTINGS['TIMEZONE'] != 'false') {
		$("div.browsesafe_heedlljjfegnjeijpnkbhpofeejflkea_timezone").each(function() { chrome.extension.sendRequest({reqtype: "update-blocked", src: window.location.href+" ("+$(this).attr('title')+"())", node: 'Spoofed Timezone'}); $(this).remove(); });
	}
