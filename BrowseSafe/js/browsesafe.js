...
if (typeof chrome.webRequest !== 'undefined') {
	...
	refreshRequestTypes();
	if (typeof chrome.webRequest !== 'undefined') {
		chrome.webRequest.onBeforeRequest.addListener(BrowseSafe, {"types": requestTypes, "urls": requestUrls}, ['blocking']);
...
function enabledfp(domainname, fptype) {
	if ((localStorage['canvas'] == 'false' && fptype == 'fpCanvas') || (localStorage['canvasfont'] == 'false' && fptype == 'fpCanvasFont') || (localStorage['audioblock'] == 'false' && fptype == 'fpAudio')) return '-1';
...
/*function genContextMenu() {
	var parent = chrome.contextMenus.create({"title": "BrowseSafe", "contexts": ["page"]});
	...
	if (storageapi && localStorage['syncenable'] == 'true') {
		...
		//if (mode == 0 || mode == 1) {
			for (var k in localStorage) {
				if (k != "version" && k != "sync" && k != "browsesafe_settings" && k != "lastSync" && k != "whiteList" && k != "blackList" && k != "whiteListCount" && k != "blackListCount" && k.substr(0, 10) != "whiteList_" && k.substr(0, 10) != "blackList_" && k.substr(0, 2) != "zb" && k.substr(0, 2) != "zw") {
					simplesettings += k+"|"+localStorage[k]+"~";
				}
			}
			simplesettings = simplesettings.slice(0,-1);
			settingssync['browsesafe_settings'] = simplesettings;
		//}
		//if (mode == 0 || mode == 2) {
		...
		//}
			...
			chrome.storage.sync.set(settingssync, function() {
				if (chrome.extension.lastError){
					...
					if (localStorage['syncnotify'] == 'true') chrome.notifications.create('syncnotify', {'type': 'basic', 'iconUrl': '../img/icon48.png', 'title': 'BrowseSafe - '+getLocale("exportsuccesstitle"), 'message': getLocale("exportsuccess")}, function(callback) { return true; } );
				}
			});
		} else {
...
function importSyncHandle(mode) {
	if (storageapi) {
		if (mode == '1' || (localStorage['sync'] == 'false' && mode == '0')) {
			chrome.storage.sync.get(null, function(changes) {
				if (typeof changes['lastSync'] !== 'undefined' && typeof changes['browsesafe_settings'] !== 'undefined' && (typeof changes['zw0'] !== 'undefined' || typeof changes['zb0'] !== 'undefined')) {
					if (changes['zw0'] != '' && changes['zw0'] != '*.googlevideo.com') { // ensure synced whitelist is not empty and not the default
						if (confirm(getLocale("syncdetect"))) {
							...
							if (localStorage['syncfromnotify'] == 'true') chrome.notifications.create('syncnotify', {'type': 'basic', 'iconUrl': '../img/icon48.png', 'title': 'BrowseSafe - '+getLocale("importsuccesstitle"), 'message': getLocale("importsuccess")}, function(callback) { return true; });
							return true;
						} else {
...
if (!optionExists("locale")) {
	...
	if (uiLang != 'en' && uiLang != 'en_GB' && uiLang != 'en_US') {
		if (typeof langs[uiLang] !== 'undefined') {
			if (confirm('BrowseSafe detected that your browser is currently set to '+langs[uiLang]+'.\r\nWould you like to use BrowseSafe in '+langs[uiLang]+'?\r\nIf you click on "Cancel", English (US) will be set.')) {
