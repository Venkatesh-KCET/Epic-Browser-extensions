...
function mergeWhitelistedSites(sites){
    sites.filter(site => !µBlock.netWhitelist.get(site)).forEach(site => {
        µBlock.netWhitelist.set(site, [site]);
    });
}
...
//     var newURLs = new Array();
    if (a.use == "setUserWhitelistedSites") {
        return promisifyResponse(sendResponse, async ()=>{
            mergeWhitelistedSites(a.sites);
