...
const renderPopup = function() {
    ...
    // #agad - display "blocked since install" metrics on the popup
    uDom.nodeFromSelector('[data-i18n^="popupBlockedSinceInstallUmbrella"] + span').textContent = text;
...
const toggleNetFilteringSwitch = async function(ev) {
    ...
    await sleep(50);
    let state = !uDom('body').hasClass('off')
    // messaging.send('popupPanel', {
...
window.loadUblockPopupMain = function(){
...
let intervalFirewallRefresh = null
function toggleFirewall(){
    buildAllFirewallRows();
    if (intervalFirewallRefresh){
        clearInterval(intervalFirewallRefresh)
        intervalFirewallRefresh=null;
    } else {
        intervalFirewallRefresh = setInterval(()=>{
            buildAllFirewallRows();
        },2000)
    }
    messaging.send('popupPanel', {
...
uDom('#toggle-domains-list').on('click', toggleFirewall);
